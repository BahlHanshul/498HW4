...
def avg_fare_by_company():
    ...
    MATCH (d:Driver)-[:WORKS_FOR]->(c:Company)
    MATCH (d)-[t:TRIP]->(:Area)
    RETURN c.name AS name, avg(t.fare) AS avg_fare
...
def area_stats():
    ...
    filtered = df.filter(col("dropoff_area") == area_id)

    stats = filtered.groupBy("dropoff_area").agg(
...
def top_pickup_areas():
    ...
    result = df.groupBy("pickup_area").agg(
        count("*").alias("trip_count")
    ).orderBy(col("trip_count").desc()).limit(n).collect()

    areas = [
        {
            "pickup_area": int(r["pickup_area"]),
